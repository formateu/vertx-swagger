...
public interface PetApi  {
    ...
    //findPetsByStatus
    default Single<ResourceResponse<List<Pet>>> findPetsByStatusWithHeader(List<String> status, User user) { 
    	return findPetsByStatus(status, user).compose(emptyHeaderWrapper()); 
    }
    
    default Single<List<Pet>> findPetsByStatus(List<String> status, User user) { 
    	return findPetsByStatus(status);
    }
    
    default Single<List<Pet>> findPetsByStatus(List<String> status) { 
    	return Single.error(new UnsupportedOperationException("Not implemented"));
    }

    
    //findPetsByTags
    default Single<ResourceResponse<List<Pet>>> findPetsByTagsWithHeader(List<String> tags, User user) { 
    	return findPetsByTags(tags, user).compose(emptyHeaderWrapper()); 
    }
    
    default Single<List<Pet>> findPetsByTags(List<String> tags, User user) { 
    	return findPetsByTags(tags);
    }
    
    default Single<List<Pet>> findPetsByTags(List<String> tags) { 
    	return Single.error(new UnsupportedOperationException("Not implemented"));
    }
