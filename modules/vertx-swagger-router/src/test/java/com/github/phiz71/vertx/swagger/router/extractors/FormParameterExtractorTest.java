...
public class FormParameterExtractorTest {
	...
	public static void beforeClass(TestContext context) {
		...
		vertxFileSystem.readFile("extractors/swaggerFormExtractor.json", readFile -> {
			if (readFile.succeeded()) {
				...
				httpServer = vertx.createHttpServer().requestHandler(swaggerRouter::accept).listen(TEST_PORT, TEST_HOST,
						listen -> {
							if (listen.succeeded()) {
								before.complete();
							} else {
		...
		eventBus.<JsonObject>consumer("POST_formdata_simple_file").handler(message -> {
			vertxFileSystem.readFile(message.body().getJsonObject("formDataRequired").getString("uploadedFileName"),
					readFile -> {
						if (readFile.succeeded()) {
	...
	public void testOkQueryArrayMulti(TestContext context) throws UnsupportedEncodingException {
		Async async = context.async();
		HttpClientRequest req = httpClient.post(TEST_PORT, TEST_HOST,
				"/formdata/array/multi?array_formdata=1&array_formdata=2&array_formdata=3");
		req.handler(response -> {
			response.bodyHandler(body -> {
				context.assertEquals(response.statusCode(), 200);
				context.assertEquals("[\"1\",\"2\",\"3\"]", body.toString());
		...
		// Construct form
		StringBuffer payload = new StringBuffer().append("array_formdata=")
				.append(esc.escape("1"))
	...
	public void testOkFormDataSimpleFile(TestContext context) {
		...
		vertxFileSystem.readFile(TEST_FILENAME, readFile -> {
			if (readFile.succeeded()) {
				...
				buffer.appendString("Content-Disposition: form-data; name=\"formDataRequired\"; filename=\""
						+ TEST_FILENAME + "\"\r\n");
